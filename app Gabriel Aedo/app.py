...
if __name__ == "__main__":
    """Aquí es donde la se muestra la ventana de la aplicacion. Se crea ademas un usuario de Personal de RR.HH en caso de que no esté en la BBDD"""
    app = App()
    conn = mongoconn.MongoConn()
    salt = bcrypt.gensalt()
    if conn.encontrar_data({"_id": "123456789"}) is None:
        conn.insertar_data({
            "_id": "123456789",
            "nombre": "super_usuario",
            "clave": bcrypt.hashpw(b"cyury123", salt),
            "sexo": "Masculino",
